...
export const OrderModal = () => {
  ...
  return (
    <div className="bg-gradient-to-br from-white via-slate-50 to-white rounded-2xl shadow-2xl w-full max-w-5xl p-8 relative animate-fadeIn text-gray-800 border border-gray-100 backdrop-blur-sm">
      ...
      >
        <VscEdit size={28} className="transition-transform duration-300 group-hover:rotate-12" />
      </button>
      ...
      {isLoading ? (
        ...
        <section className="flex flex-col items-center gap-8 animate-slideUp">
          ...
          <div className="relative">
            <h1 className="font-bold text-3xl bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent animate-pulse-slow">
              Pedido ID: {order?.id}
            </h1>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 w-full">
            ...
            <div className="bg-white rounded-xl p-4 shadow-md hover:shadow-xl transition-all duration-300 hover:-translate-y-1 border border-gray-100 group">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <p className="text-xs text-gray-500 font-medium uppercase tracking-wide">Fecha creación</p>
                  <p className="font-semibold text-gray-800">
                    {order?.fecha_pedido && new Date(order.fecha_pedido).toLocaleDateString()}
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-xl p-4 shadow-md hover:shadow-xl transition-all duration-300 hover:-translate-y-1 border border-gray-100 group">
              <div className="flex items-center gap-2">
                <span className="text-3xl group-hover:scale-110 transition-transform duration-300">🏠</span>
                <div>
                  <p className="text-xs text-gray-500 font-medium uppercase tracking-wide">Domicilio</p>
                  <p className="font-semibold text-gray-800 capitalize">{order?.domicilio}</p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-gradient-to-br from-green-50 to-emerald-50 rounded-xl p-4 shadow-md hover:shadow-xl transition-all duration-300 hover:-translate-y-1 border border-green-200 group">
              <div className="flex items-center gap-2">
                <span className="text-3xl group-hover:scale-110 transition-transform duration-300">💰</span>
                <div>
                  <p className="text-xs text-green-700 font-medium uppercase tracking-wide">Valor</p>
            ...
            <div className="bg-white rounded-xl p-4 shadow-md hover:shadow-xl transition-all duration-300 hover:-translate-y-1 border border-gray-100 group">
              <div className="flex items-center gap-2">
                ...
                <div className="flex-1">
                  <p className="text-xs text-gray-500 font-medium uppercase tracking-wide">Pago</p>
                  <span
            ...
            {order?.hora_entrega && (
              <div className="bg-white rounded-xl p-4 shadow-md hover:shadow-xl transition-all duration-300 hover:-translate-y-1 border border-gray-100 group">
                <div className="flex items-center gap-2">
                  <span className="text-3xl group-hover:scale-110 transition-transform duration-300">⏰</span>
                  <div>
                    <p className="text-xs text-gray-500 font-medium uppercase tracking-wide">Hora entrega</p>
                    <p className="font-semibold text-gray-800">
            ...
            <div className="w-full bg-blue-50 rounded-xl p-5 border-l-4 border-blue-500 shadow-md hover:shadow-lg transition-all duration-300 animate-fadeIn">
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-gray-700 capitalize leading-relaxed">{order?.observacion}</p>
                </div>
              </div>
            </div>
          )}
          ...
          {/* Productos Section */}
          <div className="w-full bg-gradient-to-br from-purple-50 to-pink-50 rounded-xl p-6 shadow-md border border-purple-200">
            <div className="flex items-center gap-3 mb-5">
              <span className="text-3xl">🛒</span>
              <h2 className="font-bold text-xl text-purple-900">Productos del Pedido</h2>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {Object.entries(productosAgrupados!).map(([categoria, productos]) => (
                <div
                  key={categoria}
                  className="bg-white rounded-lg p-4 shadow-sm hover:shadow-md transition-all duration-300 border border-purple-100 hover:border-purple-300 group"
                >
                  <h3 className="font-bold text-lg text-purple-900 mb-3 pb-2 border-b-2 border-purple-200 capitalize group-hover:text-purple-600 transition-colors duration-300">
                    {categoria}
                  </h3>
                  <ul className="space-y-2">
                    {productos.map((producto) => (
                      <li
                        ...
                        className="flex justify-between items-center text-gray-700 hover:text-purple-600 transition-colors duration-200 py-1 capitalize group/item"
                      >
                        <span className="font-medium group-hover/item:translate-x-1 transition-transform duration-200">
                          {producto.nombre}
                        </span>
                        <span className="bg-purple-100 text-purple-700 font-semibold px-3 py-1 rounded-full text-sm group-hover/item:scale-110 transition-transform duration-200">
                          ×{producto.cantidad}
                        </span>
                      </li>
                    ))}
