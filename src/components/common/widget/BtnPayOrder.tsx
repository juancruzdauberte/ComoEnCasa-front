...
export const BtnPayOrder = ({ id }: { id: number }) => {
  ...
  return (
    <button
      onClick={(e) => {
        e.stopPropagation();
        toast.warning(`¿Estás seguro de que quieres marcar como pagado el pedido #${id}?`, {
          duration: 3000,
          action: {
      ...
      type="button"
      className="p-2.5 rounded-lg bg-gradient-to-r from-green-500 to-green-600 hover:from-green-600 hover:to-green-700
    ...
    >
      <DollarSign size={18} className="group-hover/btn:scale-110 transition-transform duration-300" />
